    ...
    fn run(&self, _ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        let instruction = if trimmed.is_empty() {
            "Start an omgb multi-agent group chat.\n\n\
             Ask the user: how many agents (2-20), which model to use, and each agent's name and role.\n\
             Then use `run_terminal_cmd` to execute:\n\
             `omgb group new \"<name>\" --count <n> --model <model> --names <n1,n2,...> --roles <r1,r2,...>`\n\
             Add `--yolo` only if the user wants to auto-approve tool use.\n\n\
             Report the group id, the invite link, and how to host/join.\n\
             To host the chat, run `omgb group chat <id> --token <token> [--human-name <name>]`.\n\
             Other humans can post with `omgb group send <id> \"<message>\" --token <token> [--human-name <name>]`.\n\
