...
3. Start the host chat with `omgb group chat <id> --token <token> [--human-name <name>]`.
4. Other humans can post messages with `omgb group send <id> "<message>" --token <token> [--human-name <name>]`.
...
7. Remote clients (including the mobile app) can also create and join groups via the HTTP endpoints served by `omgb serve`: `POST /group` with the server secret creates a group; `GET/POST /group/{id}/messages` with the `x-group-token` header reads and posts messages.
8. Keep messages concise; avoid reply loops. `/quit` or `/exit` leaves the chat.
