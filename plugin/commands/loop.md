---
name: loop
description: Start an autonomous diff-driven work loop via `omgb loop`. Requires a clean git working tree and iterates until the tree is clean or the max iteration limit is reached.
---
...
5. Log each iteration to `~/.omgb/logs/loop.jsonl`.
