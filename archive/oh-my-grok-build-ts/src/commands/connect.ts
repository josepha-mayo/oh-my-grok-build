...
import { tasteContext } from "../taste.js";
import {
  scheduleListCommand,
  scheduleStopCommand,
  scheduleRunCommand,
...
async function buildLoopPrompt(
  ...
  if (iteration === 0) {
    body = original;
  } else if (iteration === total - 1) {
    body = `Wrap up and finalize. Original task: ${original}`;
  } else {
    let status: string;
    let diff: string;
    try {
      status = await gitStatusShort(cwd, GIT_DIFF_MAX_BYTES);
      diff = await gitDiff(cwd, GIT_DIFF_MAX_BYTES).catch(() => "(could not read git diff)");
    } catch (err) {
      if (isNotGitRepo(err)) {
  ...
  return tastePrefix ? `${tastePrefix}\n${body}` : body;
}
...
export async function connectCommand(options: ConnectOptions): Promise<void> {
  ...
  const tastePrefix = await tasteContext();
  ...
  while (true) {
    ...
    if (trimmed.startsWith("/loop")) {
      ...
      try {
        for (let i = 0; i < count; i++) {
          const text = await buildLoopPrompt(cwd, promptText, i, count, tastePrefix);
    ...
    if (trimmed.startsWith("/btw")) {
      ...
      const text = tastePrefix ? `${tastePrefix}\n${prompt}` : prompt;
      try {
        await client.prompt(session.sessionId, [{ type: "text", text }]);
      } catch (err) {
