...
import { tasteListCommand, tasteAddCommand, tasteRemoveCommand } from "./commands/taste.js";
...
const taste = program.command("taste").description("Manage taste/preference notes");

taste.addCommand(
  new Command("list")
    .alias("ls")
    .description("List taste notes")
    .action(async () => {
      await tasteListCommand();
    })
);

taste.addCommand(
  new Command("like")
    .description("Add a positive taste note (preference to apply)")
    .argument("<note...>")
    .action(async (note: string[]) => {
      await tasteAddCommand("like", note.join(" "));
    })
);

taste.addCommand(
  new Command("dislike")
    .description("Add a negative taste note (thing to avoid)")
    .argument("<note...>")
    .action(async (note: string[]) => {
      await tasteAddCommand("dislike", note.join(" "));
    })
);
...
taste.addCommand(
  new Command("remove")
    .alias("rm")
    .description("Remove a taste note by kind and number")
    .argument("<like|dislike>")
    .argument("<number>")
    .action(async (kind: string, num: string) => {
      ...
      await tasteRemoveCommand(kind, n);
    })
);
