...
async function injectMcpServers(raw: string): Promise<string> {
  ...
  const params = (msg.params ?? {}) as Record<string, unknown>;
  let active: ReturnType<typeof toAcpMcpServers> = [];
  try {
    active = toAcpMcpServers(await loadMcpConfig());
  } catch (err) {
    console.error("[omgb serve] failed to load MCP config:", err instanceof Error ? err.message : String(err));
  }
