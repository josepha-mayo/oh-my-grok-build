import { openSync, closeSync, readFileSync, writeFileSync, mkdirSync, appendFileSync } from "node:fs";
...
export async function spawnSubagent(
  ...
  const args = ["-p", tastedPrompt, "--model", options.model ?? DEFAULT_MODEL];
  ...
  if (proc.pid == null) {
    ...
    throw new Error(`Failed to spawn subagent "${safeName}": process did not start`);
  }
